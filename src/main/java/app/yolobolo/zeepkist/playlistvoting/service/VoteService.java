...
import app.yolobolo.zeepkist.playlistvoting.repository.ZkLevelRepo;
import app.yolobolo.zeepkist.playlistvoting.repository.ZkSessionRepo;
import app.yolobolo.zeepkist.playlistvoting.repository.ZkVoteRepo;
...
public class VoteService {
    ...
    public String createToken() {
        ZkHost host = new ZkHost();
        zkHostRepo.save(host);
        log.info("Created new host with token: {}", host.getToken());
        return host.getToken();
    }

    public ZkHost findHost(String token) {
        return zkHostRepo.findByToken(token);
    }

    // --- Session ---

    public ZkSession findActiveSession(String token) {
        ZkHost host = findHost(token);
        if (host == null) {
            log.warn("No host found for token: {}", token);
            return null;
        }
        return zkSessionRepo.findByHostId(host.getId())
                .stream()
    ...
    public ZkSession createSession(String token, String displayName) {
        ZkHost host = findHost(token);
        if (host == null) {
            log.warn("Cannot create session - no host found for token: {}", token);
            return null;
        }
        ...
        log.info("Created new session '{}' for host: {}", displayName, host.getId());
        return session;
    }
    ...
    public String vote(String token, String platformUserId, Platform platform, VoteOption option) {
        return switch (option) {
            case YES -> castVote(token, platformUserId, platform, true);
            case NO -> castVote(token, platformUserId, platform, false);
            case IDK -> castVote(token, platformUserId, platform, new Random().nextBoolean());
            case REMOVE -> removeVote(token, platformUserId, platform);
            case ABSTAIN -> abstainVote(token, platformUserId, platform);
        };
    }

    private String castVote(String token, String platformUserId, Platform platform, boolean vote) {
        ZkSession session = findActiveSession(token);
        if (session == null) return "No active session found";
        if (session.getState() == SessionState.PAUSED) return "Session is currently paused";
    ...
    public Map<String, Object> getDashboardData(String token, String hostId) {
        ...
        var sessions = zkSessionRepo.findByHostId(hostId).stream().map(s -> {
            ...
            var levels = s.getPlayedLevels().stream().map(uid -> {
                ...
                var individualVotes = lvlVotes.stream().map(v -> {
    ...
    public ZkHost findHostById(String id) {
        return zkHostRepo.findById(id).orElse(null);
    }

    public ZkHost findHostBySteamId(String steamId) {
        return zkHostRepo.findBySteamId(steamId);
    }
    ...
    public void saveHost(ZkHost host) {
        zkHostRepo.save(host);
        log.info("Host saved: {}", host.getId());
    }
}
