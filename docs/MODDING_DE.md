...
> **Wichtig:** Nur Host/HasHostPowers darf Host-Aktionen ausführen. Prüfung: `ZeepkistNetwork.IsMasterClient || ZeepkistNetwork.LocalPlayer.hasHostPowers`
...
> **Hinweis:** Das aktuelle Level-Format heißt intern `v15LevelJSON` (Klasse), ist aber das *aktuelle* Format. `v15` ist der Klassenname, keine veraltete Version.
...
Die **JoinMessage** ist eine separate Mechanik: Sie erscheint im **Chat** (nicht als Static-UI-Text). Beim Betreten der Lobby sendet der Server eine `ChatMessagePacket` mit `Sender = 0` (Server-Absender, kein echter Spieler). Diese erscheint dann ohne Spielername im Chat.
